...
def init_landmarks(init_measure, init_measure_cov, init_pose, init_pose_cov):
    ...
    beta = init_measure[::2].reshape(k,1)
    r = init_measure[1::2].reshape(k,1)
    # print("r = ", r.shape)
    xt = init_pose[0]*np.ones(k).reshape(k, 1)
    yt = init_pose[1]*np.ones(k).reshape(k, 1)
    thetat = init_pose[2]*np.ones(k).reshape(k, 1)
    
    lx = xt + r*np.cos(beta + thetat)
    ly = yt + r*np.sin(beta + thetat)
    
    landmark = np.ravel([lx, ly], 'F').reshape(2*k, 1)
    
    landmark_cov = np.zeros((12,12))
    for i in range(k):
        ...
        A = np.array([[-1, 0, -r1*np.sin(thetat1 + beta1)], [0, -1, r1*np.cos(thetat1 + beta1)]], dtype = float)

        B = np.array([[-r1*np.sin(thetat1 + beta1), np.cos(thetat1 + beta1)], [r1*np.cos(thetat1 + beta1), np.sin(thetat1 + beta1)]], dtype = float)

        
        # print("A shape = ", A.shape)
        # print("B shape = ", B.shape)
   
        measure_cov = A @ np.diag(np.ravel(init_pose)) @ A.T + B @ init_measure_cov @ B.T
        # print(measure_cov.shape)
        landmark_cov[2*i:2*i+2, 2*i:2*i + 2] = measure_cov
    
    # landmark_cov = np.kron(np.eye(k), measure_cov)
    # print("landmark_cov shape = ", landmark_cov)
...
def predict(X, P, control, control_cov, k):
    ...
    '''
    d = np.float(control[0])
    alpha = np.float(control[1])
    theta = np.float(X[2])
    # print("theta = ", theta)
    F = np.hstack((np.eye(3), np.zeros((3, 2*k))))
    # print("Fshape = ", np.shape(F))
    G = np.eye(3 + 2*k) + F.T @ np.array([[0, 0, -d*np.sin(theta)], [0, 0, d*np.cos(theta)], [0, 0, 0]]) @ F 
    L = np.array([[np.cos(theta), -np.sin(theta), 0], [np.sin(theta), np.cos(theta), 0], [0, 0, 1]])
    # print("Xshape = ", np.shape(X))
    A = np.array([[d*np.cos(theta)], [d*np.sin(theta)], [alpha]])
    # print("A shape = ", np.shape(A))
    X = X + F.T @ A
    P = G @ P @ G.T + F.T @ L @ control_cov @ L.T @ F
...
def update(X_pre, P_pre, measure, measure_cov, k):
    ...
    for i in range(k):
        ...
        # print("X_pre shape = ", np.shape(X_pre))
        lx = X_pre[3+2*i, :]
        ly = X_pre[4+2*i, :]
        mx = (lx - X_pre[0, :])[0]
        my = (ly - X_pre[1, :])[0]
        
        r_pred = np.sqrt(mx**2 + my**2)
        
        beta_pred = warp2pi(np.arctan2(my,mx)-np.float(X_pre[2]))
        # print("beta_pred = ", beta_pred)
        h_pred = np.array([[beta_pred], [r_pred]])
        # print("h_pred shape = ", np.shape(h_pred))
        
        h_true = measure[2*i:2*i+2, :]
        # print("h_true shape = ", np.shape(h_true))
        
        F = np.zeros((5, 3+2*k))
        F[0:3,0:3] = np.eye(3)
        F[3:5,3+2*i:5+2*i] = np.eye(2)
        
        H = (1/(r_pred**2)) * np.array([[my, -mx, -r_pred**2, -my, mx], 
                                        [mx/r_pred, my/r_pred, 0, -mx/r_pred, -my/r_pred]]) @ F
        
        # print("H shape = ", np.shape(H))
        #Kalman gain
        K = P_pre @ H.T @ np.linalg.inv((H @ P_pre @ H.T + measure_cov).astype(float))
        # print("K shape =", np.shape(K))
        
        X_pre = X_pre + K @ (h_true - h_pred)
...
def evaluate(X, P, k):
    ...
    plt.scatter(l_true[0::2], l_true[1::2], marker = 'x', color = 'k')
    plt.draw()
    plt.waitforbuttonpress(0)
    
    #Euclidean Distance
    x_true = l_true[::2].reshape(k, 1)
    y_true = l_true[1::2].reshape(k, 1)
    
    X_l = X[3:, :]
    x_pre = X_l[::2].reshape(k, 1)
    y_pre = X_l[1::2].reshape(k,1)
    ...
    landmark_cov = P[3:, 3:]
    delta = np.hstack([dx, dy]).reshape(k, 2)
    #Mahanalobis Distance
    
    mahanalobis = np.zeros(k)
    for i in range(k):
        e = delta[i]
        cov = landmark_cov[i*2:i*2 + 2, i*2:i*2 +2]
        dist = np.sqrt(e @ cov @ e.T) 
        mahanalobis[i] = dist
    
    
    
    print("Mahalanobis distances = ")
    print(mahanalobis.reshape(k,1))
...
def main():
    ...
    # print("k = ", k)
    # print("landmark shape: ", np.shape(landmark))
    # print("cov shape: ", np.shape(landmark_cov))
