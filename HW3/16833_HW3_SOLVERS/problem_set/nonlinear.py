...
def bearing_range_estimation(x, i, j, n_poses):
    ...
    obs = np.array([[warp2pi(np.arctan2(ly-ry, lx - rx))], [np.sqrt((lx-rx)**2 + (ly-ry)**2)]])
...
def create_linear_system(x, odoms, observations, sigma_odom, sigma_observation,
    ...
    HoA = sqrt_inv_odom @ Ho
    print(HoA.shape)

    for i in range(n_odom):
        A[2+2*i:2*i+4,2*i:2*i+4]=HoA
        obs = odometry_estimation(x,i)

        b[2+2*i:2*i+4] = np.ravel(sqrt_inv_odom @ (odoms[i,:].reshape(2,1) - odometry_estimation(x,i)))    
    ...
    for i in range(n_obs):
        ...
        A[2+2*n_odom+2*i:4+2*n_odom+2*i, 2*pose:2+2*pose] = HlA[:, 0:2]
        A[2+2*n_odom+2*i:4+2*n_odom+2*i, 2*n_poses+2*landmark:2*n_poses+2*landmark+2]=HlA[:, 2:4]

        bear = bearing_range_estimation(x, pose, landmark, n_poses)


        theta = warp2pi(observations[i,2])
        d = observations[i,3]
        obs = np.array([[theta], [d]]).reshape(2,1)

        b[2+2*n_odom+2*i:4+2*n_odom+2*i] = np.ravel(sqrt_inv_obs @ warp2pi(obs - bearing_range_estimation(x, pose, landmark, n_poses)))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--method',
        nargs='+',
        choices=['default', 'pinv', 'qr', 'lu', 'qr_colamd', 'lu_colamd','lu_bonus'],
        default=['default'],
        help='method')
