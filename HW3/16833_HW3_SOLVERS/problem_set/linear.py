...
def create_linear_system(odoms, observations, sigma_odom, sigma_observation,
	...
	print("Observations lund")
	print(observations[0:5,:])
	M = (n_odom + 1) * 2 + n_obs * 2
	N = n_poses * 2 + n_landmarks * 2

	print("no of poses", n_poses)
	print("no of odoms", n_odom)
	...
	for i in range(n_odom):
		...
		b[2+2*i:2*i+4] = sqrt_inv_odom @ odoms[i]
	...
	# A[2+2*n_odom + 1000: 2+2*n_odom + 2000, 50:250] = np.ones(200000).reshape(1000,200)
	for i in range(n_obs):
		...
		b[2+2*n_odom+2*i:4+2*n_odom+2*i] = sqrt_inv_obs @ observations[i, 2:4]
